...
#
# @param source The source location of the dbagent-x.x.x.x.zip file.  Can be any source that puppet/archive supports.
...
# @param checksum Passed to the puppet/archive module when downloading the zip.
# @param checksum_type Passed to the puppet/archive module when downloading the zip.
# @param proxy_host If specified, the HTTP proxy to use when connecting to the controller.
# @param proxy_port The port HTTP proxy port.  Required when proxy_host is set.
